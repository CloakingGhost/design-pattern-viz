...
export function FactoryMethodAnimation({ state }: FactoryMethodAnimationProps) {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-slate-50 rounded-xl overflow-hidden border border-slate-200">
      {/* 상태 메시지 영역 */}
      <div className="p-4 bg-white border-b border-slate-100 min-h-[60px] flex items-center justify-center">
      ...
      <div className="flex-1 p-8 relative flex flex-col items-center justify-center space-y-12">
        ...
        <div className="w-full max-w-2xl flex justify-between items-start z-10">
          ...
          <div className="flex flex-col items-center space-y-4">
            <div className={`p-4 rounded-xl border-2 transition-colors duration-300 ${
              financialInfo ? 'bg-blue-50 border-blue-200' : 'bg-slate-100 border-slate-200'
            }`}>
              <div className="flex items-center space-x-2 mb-2">
                <div className="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white font-bold">C</div>
              ...
              {financialInfo && (
                ...
                >
                  ...
                  <p className="text-slate-500">Card: {financialInfo.cardNumber?.slice(0, 4)}...</p>
                  <p className="text-slate-500">Email: {financialInfo.email?.split('@')[0]}...</p>
                </motion.div>
              )}
            </div>
          ...
          <div className="flex flex-col items-center">
             <div className="p-4 rounded-xl border-2 border-dashed border-slate-300 bg-slate-50/50 mb-4 opacity-50">
                <span className="text-sm font-semibold text-slate-400">{'<<Abstract>>'}</span>
             ...
             <div className="p-4 rounded-xl border-2 border-dashed border-slate-300 bg-slate-50/50 opacity-50">
                <span className="text-sm font-semibold text-slate-400">{'<<Interface>>'}</span>
        ...
        <div className="w-full max-w-4xl grid grid-cols-2 gap-8 relative">
          ...
          <div className={cn(
            ...
            <AnimatePresence>
              {currentFactory === 'CreditCardPaymentFactory' && paymentType === 'CreditCardPayment' && (
                <motion.div
                ...
                >
                  ...
                  <span className="font-bold text-slate-700">CreditCardPayment</span>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          ...
          <div className={cn(
            ...
            <div className="font-bold text-sky-700 mb-4 flex items-center space-x-2">
             ...
             <AnimatePresence>
              {currentFactory === 'PayPalPaymentFactory' && paymentType === 'PayPalPayment' && (
                <motion.div
                  initial={{ opacity: 0, scale: 0.5 }}
                  animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0, scale: 0.5 }}
                ...
                >
                  ...
                  <span className="font-bold text-slate-700">PayPalPayment</span>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
             ...
             <motion.div
                ...
                className={cn(
                  "absolute bottom-0 p-3 rounded-lg shadow-xl border-2 z-20 flex items-center space-x-2",
                   createdPayment === 'CreditCardPayment' ? "bg-indigo-600 border-indigo-700 text-white" : "bg-sky-600 border-sky-700 text-white"
                )}
             >
                {createdPayment === 'CreditCardPayment' ? <CreditCard size={16}/> : <Banknote size={16}/>}
